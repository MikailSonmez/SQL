...
CREATE TABLE calisanlar 
    ...
    select * from calisanlar , markalar where isyeri = marka_isim;
...
/* ----------------------------------------------------------------------------
  ...
  -----------------------------------------------------------------------------*/ 
	...
	-- '=' yerine 'in' kullandik cunki birkac tanesine (veya) esit olabilir
...
/* -----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/
select marka_isim, calisan_sayisi, ( select round(avg(maas)) from calisanlar where marka_isim=isyeri ) as ortalama_maas
from markalar;  
-- round yaparsak yuvarlar yoksa kusuratli verir
...
/* ----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/  
 select marka_isim, calisan_sayisi, ( select min(maas) from calisanlar where marka_isim=isyeri ) as min_maas,( select max(maas) from calisanlar where marka_isim=isyeri ) as max_maas
from markalar; 
