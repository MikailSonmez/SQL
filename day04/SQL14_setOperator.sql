...
union 																				-- union all, tekrarlananlari da gosterir
select maas,sehir from personel where sehir='Istanbul'
order by maas desc;

/* -----------------------------------------------------------------------------
  ORNEK3: Honda,Ford ve Tofas'ta calisan (ortak, şu anlık yok) isimde personel varsa listeleyin
------------------------------------------------------------------------------*/ 
...
union
select isim, maas, sirket from personel where sirket <> 'Honda';    -- != bu sekilde de yazabiliriz

/* -----------------------------------------------------------------------------
  ORNEK5: Ismi Mehmet Ozturk olanlari ve Istanbul'da calismayanlarin isimlerini ve 
...
------------------------------------------------------------------------------*/
select id, sehir from personel where sehir in ('Istanbul', 'Ankara')
union
select id,cocuk_sayisi from Personel_bilgi where cocuk_sayisi in (2,3);

-- sirketlerden grupla sirketlerin calisan sisimlerini say
